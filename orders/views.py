...
def order_create(request):
    ...
    transport_cost = round((2.99 + (cart_qty // 10) * 1.5), 2)
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            charge = stripe.Charge.create (customer = customer, amount = int(order.get_total_cost() * 100), currency='usd', description = order)
    ...
    return render(request, 'order_create.html', {'cart': cart, 'order_form': order_form, 'transport_cost': transport_cost})